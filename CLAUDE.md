...
**Precision gate.** SAM 2 emits a full mask for every seeded object every frame — even when that player is occluded or has left — so the mask drifts onto a neighbor/background and yields a phantom box (false positive). `track_with_sam2` therefore keeps a propagated box only if it overlaps a real per-frame YOLO person detection, one object per detection (`gate_detector`/`--gate-iou`; disable with `--no-gate`). Measured on the `eval/` clips, this cut sam2 false positives ~9× (27→3 on pickup2) and lifted **IDF1 0.90→0.97 / MOTA 0.72→0.88** for a small recall cost — flipping sam2 from roughly tied with the tracklets backbone to clearly ahead.

Trade-offs vs. `tracklets`: far better identity persistence on busy footage (verified — 6 seeds tracked across *all* frames of a gym clip, where the tracklet backbone fragmented the same footage into ~10 ids), but the predictor holds the whole clip in memory (cost ≈ objects × frames × `imgsz²`), so it is bounded to a **short** clip — `run_offline.py` auto-extracts a ~120-frame subclip from the reference frame (`pipeline.subclip`). Residual risks: mask bleed between players in a tight scrum; identity drift through long full occlusion; and (with the gate on) recall is now capped by the YOLO detector. Tested envelope on an M3 Pro (24 GB): 6 objects × 120 frames × `imgsz=640` ≈ 60 s; raise frames/imgsz cautiously (MPS OOM beyond ~that).
